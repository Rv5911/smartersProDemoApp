...
function restoreMoviesNavigationState() {
    try {
        const saved = localStorage.getItem("moviesNavState");
        let hasState = false;

        if (saved) {
            const state = JSON.parse(saved);
            moviesNavigationState.currentCategoryIndex =
                typeof state.currentCategoryIndex === "number" ?
                state.currentCategoryIndex :
                0;
            moviesNavigationState.currentCardIndex =
                typeof state.currentCardIndex === "number" ? state.currentCardIndex : 0;
            moviesNavigationState.lastFocusedCategory =
                typeof state.lastFocusedCategory === "number" ?
                state.lastFocusedCategory :
                0;
            moviesNavigationState.lastFocusedCard =
                typeof state.lastFocusedCard === "number" ? state.lastFocusedCard : 0;
            moviesNavigationState.focus = state.focus || "categories";
            hasState = true;
        }

        // If we are coming back from MovieDetailPage, prefer the explicitly
        // stored category/card indices so we can restore focus to the
        // exact card that opened the detail page (same behavior as Series).
        const prevPage = localStorage.getItem("previousPage");
        if (prevPage === "movieDetailPage") {
            const storedCategoryRaw = localStorage.getItem("moviesCategoryIndex");
            const storedCardRaw = localStorage.getItem("moviesCardIndex");

            const storedCategory = storedCategoryRaw ?
                parseInt(storedCategoryRaw, 10) :
                NaN;
            const storedCard = storedCardRaw ? parseInt(storedCardRaw, 10) : NaN;

            if (!Number.isNaN(storedCategory) && storedCategory >= 0) {
                moviesNavigationState.currentCategoryIndex = storedCategory;
            }
            if (!Number.isNaN(storedCard) && storedCard >= 0) {
                moviesNavigationState.currentCardIndex = storedCard;
            }
            moviesNavigationState.focus = "categories";
            hasState = true;
        }
...
function MoviesPage() {
    ...
    `;
        ...
        const navFocus = localStorage.getItem("navigationFocus"); // Define navFocus here

        if (!isSearchFocused) {
            // Fix: Default to 'navbar' focus when entering the page.
            // Only focus content ('moviesPage') if we are returning from the Detail Page.
            if (navFocus === "moviesPage" || previousPageVal === "movieDetailPage") {
                localStorage.setItem("navigationFocus", "moviesPage");
            } else {
                localStorage.setItem("navigationFocus", "navbar");
    ...
    if (!isSearchFocused && navFocus !== "sidebar") {
        // Keep existing moviesPage focus (e.g. when returning from MovieDetailPage)
    ...
    setTimeout(async function() {
        ...
        let hasMoreCategories = false;
        for (
            let i = initialCategories.length; i < window.allMoviesCategories.length; i++
        ) {
