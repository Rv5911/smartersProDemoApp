function LivePage() {
  ...
  const navigateUp = () => {
    if (focusedSection === "sidebar") {
      ...
      if (buttonFocusIndex === 1) {
        buttonFocusIndex = 0; // Move from remove to heart
  ...
  const navigateDown = () => {
    if (focusedSection === "sidebarSearch") {
      ...
      if (buttonFocusIndex === -1) {
        buttonFocusIndex = 0; // Move from card to heart
      } else if (
        buttonFocusIndex === 0 &&
        selectedCategoryId === "channelHistory"
      ) {
        buttonFocusIndex = 1; // Move from heart to remove
      } else {
        // IMPROVEMENT: On Arrow Down from Heart/Remove, focus next card
        buttonFocusIndex = -1; // Reset to card
        if (channelIndex < filteredStreams.length - 1) {
