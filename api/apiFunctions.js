const TMBD_API_KEY = window.TMBD_API_KEY
  ? window.TMBD_API_KEY
  : "a21eeaca44af5d2a4349214ecba1b338";
...
function resetLoadingPercentage() {
  ...
  if (loadingOverlay) {
    loadingOverlay.style.background = "rgba(0, 0, 0, 0.92)";
...
function updateLoadingPercentage(
  targetPercentage,
  message = "Loading your playlist content...",
) {
  const progressElement = document.getElementById("loading-progress");
  const messageElement = document.getElementById("loading-message");
...
async function loginApi(
          ...
          const closeDialog = () => {
            ...
            if (previousActiveElement) {
              ...
              if (
                ...
                (previousActiveElement.type === "text" ||
                  previousActiveElement.type === "password" ||
                  previousActiveElement.type === "url")
              ) {
                const val = previousActiveElement.value;
                previousActiveElement.setSelectionRange(val.length, val.length);
              }
            }
          };
  ...
  try {
    ...
    for (let i = 0; i < dnsToCheck.length; i++) {
      ...
      // updateLoadingPercentage(null, "Verifying credentials...");
        ...
        if (!response.ok) {
          ...
          if (isSingleDns) {
            ...
            setTimeout(() => {
              ...
              Toaster.showToast(
                "error",
                // `Login Failed: Status ${response.status} Invalid Credentials`,
                `Invalid Credentials`,
              );
            }, 500);
            return null;
